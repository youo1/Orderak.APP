...
export function ErrorState({ error, retry }: { error: { message: string }; retry: () => void }) { return <div className="state-card error"><h2>Could not load this section</h2><p>{error.message}</p><button className="button" onClick={retry}>Try again</button></div>; }
