...
export function RefineResourcePage({ section }: { section: Section }) {
  ...
  return <CanAccess resource={section.id} action="list">
    <PageHeader title={section.label} description={section.description} actions={<>{action && auth.can(action.permission) && <button className="button primary" onClick={() => setActionOpen(true)}><Plus size={16} /> {action.label}</button>}<button className="button" onClick={refetch}><RefreshCw size={16} /> Refresh</button></>} />
