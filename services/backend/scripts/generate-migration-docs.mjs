...
const descriptions = {
  ...
  "055_media_objects.sql": [
    "Records which store uploaded each R2 object and when, so media nothing references any more can be found. The only `orderak_media.delete()` in the tree cleared a whole prefix on account deletion, so every replaced logo, cover and product photo stayed in the bucket permanently - and stayed fetchable at its `/media/` URL, which is a weaker privacy position than the retention matrix takes everywhere else.",
    "Provenance only, deliberately: no foreign key back to the row using the image. An upload is a separate request from the store update or product sync that later names its URL, so an object that is never attached to anything has no replacement event to hook - and that is the larger half of the problem. Whether an object is still referenced is asked at sweep time of the columns that would reference it (`sellers.logo_url`, `sellers.cover_url`, `products.image_url`), which has one correct answer at any moment and cannot drift.",
