...
async function stats(env: AdminWorkerEnv): Promise<Response> {
	...
	const revenue = (await env.orderak_db.prepare("SELECT COALESCE(SUM(amount_piasters),0) AS r FROM subscriptions WHERE status='active'").first()) as {r:number};
	const couponUses = (await env.orderak_db.prepare("SELECT COUNT(*) AS c FROM coupon_uses").first()) as {c:number};
	const pendingPayouts = (await env.orderak_db.prepare("SELECT COALESCE(SUM(commission_piasters),0) AS r FROM referrals WHERE status='qualified'").first()) as {r:number};
	return jsonResponse({ok:true,active_paid_subscriptions:activeSubs.c,total_sellers:totalSellers.c,subscriptions_by_plan:byPlan,monthly_revenue_piasters:revenue.r,coupon_uses:couponUses.c,pending_commission_piasters:pendingPayouts.r});
}
...
async function upsertPlan(request: Request, env: AdminWorkerEnv, admin: AdminClaims): Promise<Response> {
	...
	const price = Math.max(0,Math.floor(Number(b.price_piasters)||0));
...
async function setAffiliate(request: Request, env: AdminWorkerEnv, admin: AdminClaims): Promise<Response> { const b = (await request.json().catch(()=>({}))) as Record<string,unknown>; await env.orderak_db.prepare("INSERT INTO affiliate_settings(id,commission_type,commission_value,referral_bonus_type,referral_bonus_value,min_payout_piasters,payout_info,updated_at) VALUES(1,?,?,?,?,?,?,datetime('now')) ON CONFLICT(id) DO UPDATE SET commission_type=excluded.commission_type,commission_value=excluded.commission_value,referral_bonus_type=excluded.referral_bonus_type,referral_bonus_value=excluded.referral_bonus_value,min_payout_piasters=excluded.min_payout_piasters,payout_info=excluded.payout_info,updated_at=datetime('now')").bind(b.commission_type==="fixed"?"fixed":"percentage",Math.max(0,Math.floor(Number(b.commission_value)||0)),b.referral_bonus_type==="fixed"?"fixed":"percentage",Math.max(0,Math.floor(Number(b.referral_bonus_value)||0)),Math.max(0,Math.floor(Number(b.min_payout_piasters)||0)),String(b.payout_info??"")).run(); await auditDb(env,admin,"admin.affiliate_updated",{entity:"affiliate_settings",entity_id:1},request); return jsonResponse({ok:true}); }
