...
describe("privacy retention cleanup", () => {
	it("removes or de-identifies technical data older than 30 days", async () => {
		await env.orderak_db.batch([
			...
			env.orderak_db.prepare("INSERT INTO email_template_history(changed_ip,changed_at) VALUES('203.0.113.4',datetime('now','-31 days'))"),
			env.orderak_db.prepare("INSERT INTO admin_sessions(id,expires_at,created_at) VALUES('expired',datetime('now','-1 day'),datetime('now','-2 days'))"),
