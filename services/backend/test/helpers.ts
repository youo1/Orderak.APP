...
export async function createEntitlementSchema(): Promise<void> {
	const statements = [
		...
		`CREATE TABLE IF NOT EXISTS play_product_mappings(id TEXT PRIMARY KEY,plan_id TEXT,product_id TEXT,base_plan_id TEXT,package_name TEXT,active INTEGER,last_synced_at TEXT,price_snapshot_json TEXT,created_at TEXT DEFAULT (datetime('now')),UNIQUE(package_name,product_id,base_plan_id))`,
